...
def upload_images(request: Request, files: List[UploadFile] = File(...)):
    ...
    # Для каждого состава сохраняем список фамилий
    squads = []
    for fname in images_for_chart:
        img_path = os.path.join(UPLOAD_DIR, fname)
        try:
            ...
            pass
    # --- Новый анализ: коэффициент связи пар ---
    # Считаем для каждого игрока, в скольких составах он был
    ...
    # Сортировка по убыванию процента совместного появления, затем по together
    ...
    """
    # Оставляем только топ-5 по проценту и количеству вместе (но если у нескольких пар одинаковые значения — показываем все такие)
    top_n = 5
    cutoff = None
    if pair_stats:
        # Получить пороговые значения для попадания в топ
    ...
    # --- Уникальные “разделённые” игроки ---
    # Считаем для каждой пары: если оба появлялись >=2 раза и ни разу не были вместе
    separated_pairs = []
    ...
    # --- Матрица корреляций (heatmap по коэффициенту Жаккара) ---
    # Берём только игроков, кто был хотя бы 2 раза
    matrix_players = [name for name, cnt in player_counts.items() if cnt >= 2]
    matrix_idx = {name: i for i, name in enumerate(matrix_players)}
    n = len(matrix_players)
    jaccard = [[0.0]*n for _ in range(n)]
    # Для каждого игрока — множества номеров составов, где он был
    player_squads = {name: set() for name in matrix_players}
    for squad_idx, squad in enumerate(squad_sets):
        for name in squad:
            if name in player_squads:
                player_squads[name].add(squad_idx)
    for i, a in enumerate(matrix_players):
        for j, b in enumerate(matrix_players):
            if i == j:
                jaccard[i][j] = 1.0
            else:
                sa, sb = player_squads[a], player_squads[b]
                if sa or sb:
                    jaccard[i][j] = len(sa & sb) / len(sa | sb)
                else:
                    jaccard[i][j] = 0.0
    # SVG heatmap (простая, без зависимостей)
    cell_size = 36
    svg_w = cell_size * n + 150
    svg_h = cell_size * n + 80
    def color(val):
        # от светло-серого (0) до синего (1)
        base = 230 - int(130 * val)
        return f'rgb({base},{base},{255})'
    heatmap_svg = (
        "<h3 style='margin-top:2.2em;font-size:1.04em;color:#2a2d34;'>Матрица корреляций (коэффициент Жаккара)</h3>"
        f"<svg width='{svg_w}' height='{svg_h}' style='background:#fafbfc;border-radius:8px;box-shadow:0 2px 8px #e3eaf2;margin-bottom:2em;'>"
    )
    # Подписи по X
    for i, name in enumerate(matrix_players):
        heatmap_svg += f"<text x='{150 + i*cell_size + cell_size//2}' y='32' text-anchor='middle' font-size='13' fill='#345'>{name}</text>"
    # Подписи по Y
    for i, name in enumerate(matrix_players):
        heatmap_svg += f"<text x='140' y='{70 + i*cell_size + cell_size//2}' text-anchor='end' font-size='13' fill='#345'>{name}</text>"
    # Клетки
    for i in range(n):
        for j in range(n):
            val = jaccard[i][j]
            fill = color(val)
            heatmap_svg += f"<rect x='{150 + j*cell_size}' y='{50 + i*cell_size}' width='{cell_size-2}' height='{cell_size-2}' fill='{fill}' rx='6'/><text x='{150 + j*cell_size + cell_size//2}' y='{50 + i*cell_size + cell_size//2 + 6}' text-anchor='middle' font-size='13' fill='#223'>{val:.2f}</text>"
    heatmap_svg += "</svg>"
