...
def upload_images(request: Request, files: List[UploadFile] = File(...)):
    ...
    logger = logging.getLogger("fantasyocr")
    summary_blocks = []
    MAX_FILES = 5
    if len(files) > MAX_FILES:
        return templates.TemplateResponse("index.html", {"request": request, "extracted_text": None, "images": get_uploaded_images(UPLOAD_DIR), "error": f"Можно загрузить не более {MAX_FILES} файлов за раз."})
    try:
        ...
        for file in files:
            try:
                ...
                if image.width < min_width:
                    ...
                    image = image.resize((min_width, int(image.height * ratio)))
                filename = file.filename
                save_path = os.path.join(UPLOAD_DIR, filename)
                image.save(save_path)
                logger.info(f"Сохранён файл: {save_path}")
            except Exception as e:
                logger.error(f"Ошибка при сохранении файла {file.filename}: {e}")
                return templates.TemplateResponse("index.html", {"request": request, "extracted_text": None, "images": get_uploaded_images(UPLOAD_DIR), "error": f"Ошибка при сохранении файла {file.filename}: {e}"})
        # --- EasyOCR + подсчёт фамилий ---
        all_found_names = []
        for img_path in get_uploaded_images(UPLOAD_DIR):
            try:
                ...
                if easyocr is not None:
                    try:
                        ...
                        logger.info(f"EasyOCR успешно завершён для {img_path}")
                    except Exception as e:
                        logger.error(f"[ERROR] EasyOCR: {e}")
                all_names = extract_player_names("\n".join(easy_txt))
                all_found_names.extend(all_names)
            except Exception as e:
                logger.error(f"Ошибка при обработке изображения {img_path}: {e}")
                return templates.TemplateResponse("index.html", {"request": request, "extracted_text": None, "images": get_uploaded_images(UPLOAD_DIR), "error": f"Ошибка при обработке изображения {img_path}: {e}"})
        # --- Барчарт ---
